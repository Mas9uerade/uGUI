...
{
    public abstract class ForkedHorizontalOrVerticalLayoutGroup: ForkedLayoutGroup
    {
    ...
    public bool childScaleHeight { get { return m_ChildScaleHeight; } set { SetProperty(ref m_ChildScaleHeight, value); } }
        ...
        [SerializeField] protected float m_MaxStrech = 0;
        /// <summary>
        /// The maxStrech between layout elements in the layout group.
        /// </summary>
        public float maxStrech { get { return m_MaxStrech; } set { SetProperty(ref m_MaxStrech, value); } }
        /// <summary>
        ...
        protected void CalcAlongAxis(int axis, bool isVertical)
    {
        ...
        for (int i = 0; i < rectChildren.Count; i++)
        {
            ...
            else
            {
                    if (axis == 0)
                    {
                        totalMin = Mathf.Max(totalMin, child.anchoredPosition[axis] + min * child.pivot[axis] + spacing + padding.right);
                        totalPreferred = Mathf.Max(totalPreferred, child.anchoredPosition[axis] + preferred * child.pivot[axis] + spacing + padding.right);
                    }
                    else
                    {
                        totalMin = Mathf.Max(totalMin, Mathf.Abs( child.anchoredPosition[axis]) + min * child.pivot[axis] + padding.bottom + padding.top);
                        totalPreferred = Mathf.Max(totalPreferred, Mathf.Abs( child.anchoredPosition[axis]) + preferred * child.pivot[axis] + padding.bottom+ padding.top);
                    }
                    //totalMin += min + spacing;
                    //totalPreferred += preferred + spacing;
                    Debug.LogWarningFormat("Index : {0}, axis:{1}, totalMin:{2}, preferred:{3}", i, axis, totalMin, totalPreferred);
    ...
    protected void SetChildrenAlongAxis(int axis, bool isVertical)
    {
        ...
        /// <param name="axis">The axis to handle. 0 is horizontal and 1 is vertical.</param>
        /// <param name="isVertical">Is this group a vertical group?</param>
        protected void SetChildrenAlongAxisExpand(int axis)
        {
            float size = rectTransform.rect.size[axis];
            bool controlSize = (axis == 0 ? m_ChildControlWidth : m_ChildControlHeight);
            bool useScale = (axis == 0 ? m_ChildScaleWidth : m_ChildScaleHeight);
            ...
            float posAxis = (axis == 0 ? padding.top : padding.left);
            float posSide = (axis == 0 ? padding.left : padding.top);
            float sizeSide = 0;
            ...
            if (surplusSpace > 0)
            {
                if (GetTotalFlexibleSize(axis) == 0)
                {
                    posAxis = GetStartOffset(axis, GetTotalPreferredSize(axis) - (axis == 0 ? padding.horizontal : padding.vertical));
                }
                else if (GetTotalFlexibleSize(axis) > 0)
                {
                    itemFlexibleMultiplier = surplusSpace / GetTotalFlexibleSize(axis);
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
                ...
                if (controlSize)
                {
                    SetChildAlongAxisWithScale(child, axis, posAxis, childSize, scaleFactor);
                }
                else
                {
                    //Debug.LogFormat("Index : {0}, x:{1}, y:{2}", i, posAxis + childSize * scaleFactor, posSide);
                    if (posAxis + childSize * scaleFactor >= maxStrech)
                    {
                        posAxis = (axis == 0 ? padding.top : padding.left);
                        posSide += sizeSide + spacing;  
                        sizeSide = child.sizeDelta[axis == 0 ? 1 : 0];
                    }
                    float offsetInCell = (childSize - child.sizeDelta[axis]) * alignmentOnAxis;
                    Debug.LogFormat("Index : {0}, x:{1}, y:{2}", i, posAxis + childSize * scaleFactor, posSide);
                    SetChildPositionWithScale(child, new Vector2(posAxis + offsetInCell, posSide), scaleFactor);

                    //SetChildAlongAxisWithScale(child, axis, posAxis + offsetInCell, scaleFactor);
                    //SetChildAlongAxisWithScale(child, axis == 0 ? 1 : 0, posSide, scaleFactor);

                }
                
                sizeSide = Mathf.Max(sizeSide, child.sizeDelta[axis == 0 ? 1 : 0]);
                posAxis += childSize * scaleFactor + spacing;
            }
